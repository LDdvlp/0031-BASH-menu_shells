...
setup() {
  ...
  export HOME="${BATS_TEST_TMPDIR}/home_linux"
  mkdir -p "$HOME"
}
...
@test "install + run menu on Linux (simulé)" {
  run bash -c "cd \"$PROJECT_DIR\" && MENU_SHELLS_FORCE_ENV='Linux/Unix' ./install.sh"
  [ "$status" -eq 0 ]
  ...
  run bash -c "MENU_SHELLS_FORCE_ENV='Linux/Unix' \"$PROJECT_DIR/scripts/menu.sh\" <<<'q'"
  [ "$status" -ge 0 ]
  [[ "$output" == *"1) Bash"* ]]
  [[ "$output" == *"q) Quitter"* ]]
}
