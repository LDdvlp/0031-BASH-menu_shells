...
setup() {
  ...
  export HOME="${BATS_TEST_TMPDIR}/home_wsl"
  mkdir -p "$HOME"
}
...
@test "install + run menu on WSL (simulé)" {
  run bash -c "cd \"$PROJECT_DIR\" && MENU_SHELLS_FORCE_ENV='WSL' ./install.sh"
  [ "$status" -eq 0 ]
  ...
  run bash -c "MENU_SHELLS_FORCE_ENV='WSL' \"$PROJECT_DIR/scripts/menu.sh\" <<<'q'"
  [ "$status" -ge 0 ]
  [[ "$output" == *"1) Bash"* ]]
  [[ "$output" == *"2) Zsh"* ]]
  [[ "$output" == *"3) Infos"* ]]
